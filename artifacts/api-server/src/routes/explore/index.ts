...
router.post("/explore/walk-narration", async (req, res) => {
  ...
  const {
    ...
    crossStreets,
    evidenceRef,
    // Read by the narration-time JIT A3 / evidence-floor path below
    // (resolveNarrationEvidence) when evidenceRef is absent — see the same
    // fields' doc comments on /explore/walk-narration-audio. Not used for
    // the narration cache key, ranking, or any other content decision.
    subjectId,
    candidateSource,
    wikipediaTag,
  } = parsed.data;
  ...
  const factsKeyPart = (facts || [])
    ...
    .sort()
    .join("|");
  // v22 (2026-09-05): bumped in lockstep with /explore/walk-narration-audio's
  ...
  // the same key. This route's live-call branch can now also feed the
  // narration LLM a JIT-enriched facts list and is gated by the new
  // evidence floor (evidence-floor fix), a real content-input change for
  // the same nominal cache key that deserves a fresh cache slot. The
  // version must stay identical across both routes for cache-sharing to
  // keep working.
  const narrationCacheKey = `narration:v22:${placeName.toLowerCase()}|${(category || "").toLowerCase()}|${summary.slice(0, 80).toLowerCase()}|${factsKeyPart}`;
  ...
  // Narration-time JIT A3, plus the curated-before-Wikipedia evidence order
  // and the deterministic evidence floor (2026-09-05 evidence-floor fix) —
  // see resolveNarrationEvidence/hasMinimumNarrationEvidence above
  // /explore/walk-narration-audio. This route previously had neither: on
  // native devices the client falls back here from /walk-narration-audio on
  // any audio failure (including a floor-suppressed empty response), so
  // without the same gate here that fallback would silently defeat
...
- If you're not certain of a detail, say "supposedly" or "according to local accounts" rather than stating it as fact.`,
        ...
        {
          role: "user",
          content: `I'm walking past "${placeName}" (${category || "place"}).${locationContext} Here's what's interesting: ${summary}${(narrationFacts || []).length ? ` Also: ${(narrationFacts || []).join(" ")}` : ""}. Give me a brief, natural narration.`,
        },
      ],
    })
    // @end-prompt-region walk-narration
...
export async function runNarrationJitEvidence(
  ...
  if (candidateSource === "osm") {
    // Evidence-floor fix (2026-09-05): check curated evidence for the exact
    // candidate BEFORE the wikipediaTag check. This is the same
    // getApprovedCuratedEntry lookup the "streetlit" branch above already
    // uses — subjectId equals the OSM element id for osm candidates (see
    // WalkModeContext.tsx's `place.osmId ?? place.streetlitId`), so an
    // approved curated entry keyed by that OSM id resolves correctly here.
    // Fixes Green Room: an OSM candidate with no wikipediaTag but an
    // approved curated entry must not be routed straight to the
    // no-Wikipedia-tag skip.
    if (subjectId) {
      const curated = getApprovedCuratedEntry(subjectId);
      if (curated) {
        return { outcome: "curatedSuccess", factText: curated.evidence.text };
...
// Shared evidence-resolution step for both narration routes (2026-09-05
// evidence-floor fix). Orchestrates narration-time JIT A3
// (runNarrationJitEvidence — curated-before-Wikipedia, see above) and
// returns the enriched facts list plus outcome detail for the temporary
// diagnostics below. Falls back to the unmodified request facts on any
// skip/timeout/failure, identical to today's behavior when JIT does not
// apply. Extracted here (rather than duplicated) because both
// /walk-narration and /walk-narration-audio must now run it, so that the
// evidence floor below has the same fully-resolved input on either route.
type NarrationEvidenceResolution = {
  narrationFacts: string[] | undefined;
  jitOutcome: NarrationJitOutcome;
...
export async function resolveNarrationEvidence(params: {
...
}): Promise<NarrationEvidenceResolution> {
  const {
    evidenceRef,
    candidateSource,
    subjectId,
  ...
  return {
    narrationFacts: jitFactText ? [...(facts || []), jitFactText] : facts,
    jitOutcome,
    jitAttempted,
    jitMs,
...
// Evidence floor (2026-09-05). Deterministic, structural presence gate
// applied to the FINAL resolved narration input — after curated/JIT/fallback
// evidence resolution is complete, before the copy-generation prompt is
// built. This is an evidence-PRESENCE floor, not a narration-quality
// judgment: it does not infer story quality from years, civic vocabulary,
// word count, or any other lexical heuristic (that broader judgment is
// classifyDiscoveryTier's job elsewhere in ranking/eligibility, and is
// deliberately not reused here). Priority order:
//   1. Curated evidence resolved successfully → proceed.
//   2. Wikipedia JIT resolved successfully → proceed.
//   3. Otherwise, suppress only when the final summary is the exact known
//      placeholder/default shape (PLACEHOLDER_SUMMARY) AND narrationFacts
//      contains no non-empty entry.
//   4. Otherwise (e.g. a non-placeholder but still generic fallback summary
//      with no facts) → proceed. This is a known, intentional limitation of
//      this narrow pass — see narrationEvidenceFloor.test.ts.
export function hasMinimumNarrationEvidence(input: {
  jitOutcome: NarrationJitOutcome;
  summary: string;
  narrationFacts: string[] | undefined;
}): boolean {
  if (
    input.jitOutcome === "curatedSuccess" ||
    input.jitOutcome === "wikipediaSuccess"
  ) {
    return true;
  }
  const hasMeaningfulFact = (input.narrationFacts ?? []).some(
    (fact) => fact.trim().length > 0,
  );
  const isPlaceholderShape =
    input.summary === PLACEHOLDER_SUMMARY && !hasMeaningfulFact;
  return !isPlaceholderShape;
}
...
router.post("/explore/walk-narration-audio", async (req, res) => {
  ...
  const factsKeyPart = (facts || [])
    ...
    .sort()
    .join("|");
  // v22 (2026-09-05): bumped in lockstep with /explore/walk-narration's own
  // narrationCacheKey — see that route's comment for why (evidence-floor
  // fix). This route's own reason: the live-call branch below can now also
  // find curated evidence before the Wikipedia-tag check and is gated by
  // the new evidence floor, a real content-input change for the same
  // nominal cache key that deserves a fresh cache slot.
  const narrationCacheKey = `narration:v22:${placeName.toLowerCase()}|${(category || "").toLowerCase()}|${summary.slice(0, 80).toLowerCase()}|${factsKeyPart}`;
  ...
  if (cachedNarration) {
    ...
    if (existingNarrationFlight) {
      ...
      // Narration-time JIT A3, plus the curated-before-Wikipedia evidence
      // order (evidence-floor fix, 2026-09-05) — see resolveNarrationEvidence
      // above. Only attempted when discover-time A3 evidence (evidenceRef)
      // is absent and the candidate carries trustworthy Build 14 identity
      // (subjectId/candidateSource/wikipediaTag — lat/lon are advisory only
      // and never used as a lookup key). Deliberately placed outside the
      // marked prompt-region below: this decides WHAT goes into the facts
      // list fed to the existing narration prompt, it does not change the
