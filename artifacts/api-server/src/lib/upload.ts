...
export function createUpload(
  ...
  if (
    ...
    const message = `createUpload maxFields (${options.maxFields}) is below UPLOAD_MAX_FIELDS (${UPLOAD_MAX_FIELDS}); this endpoint enforces a stricter cap than the global baseline, which may surprise callers that expect the documented baseline limit`;
    const context = {
  ...
  if (options?.fieldSizeOverride !== undefined) {
    ...
    if (options.fieldSizeOverride < UPLOAD_FIELD_SIZE) {
      const message = `createUpload fieldSizeOverride (${options.fieldSizeOverride} bytes) is below UPLOAD_FIELD_SIZE (${UPLOAD_FIELD_SIZE} bytes); this endpoint enforces a stricter cap than the global baseline, which may surprise callers that expect the documented baseline limit`;
      const context = {
        fieldSizeOverride: options.fieldSizeOverride,
        UPLOAD_FIELD_SIZE,
      };
  ...
  if (options?.fieldNameSizeOverride !== undefined) {
    ...
    if (options.fieldNameSizeOverride > UPLOAD_FIELD_NAME_SIZE) {
      const message = `createUpload fieldNameSizeOverride (${options.fieldNameSizeOverride} bytes) exceeds UPLOAD_FIELD_NAME_SIZE (${UPLOAD_FIELD_NAME_SIZE} bytes); the per-endpoint field-name-size cap is looser than the global baseline, so other endpoints may enforce a tighter limit than this one`;
      const context = {
        fieldNameSizeOverride: options.fieldNameSizeOverride,
        UPLOAD_FIELD_NAME_SIZE,
      };
