...
describe("createUpload — fileSizeOverride vs UPLOAD_BODY_LIMIT check", () => {
  ...
  it("does not warn when fileSizeOverride is below UPLOAD_BODY_LIMIT bytes", async () => {
    ...
    vi.doMock("../config", () => ({
      UPLOAD_BODY_LIMIT: "10mb",
      UPLOAD_STRICT_CONFIG: false,
      // Set UPLOAD_MAX_FILE_SIZE <= fileSizeOverride so only the body-limit
      // guard is under test here, not the below-baseline guard.
...
// ---------------------------------------------------------------------------
// createUpload — fieldNameSizeOverride vs UPLOAD_BODY_LIMIT guard
// ---------------------------------------------------------------------------

describe("createUpload — fieldNameSizeOverride vs UPLOAD_BODY_LIMIT check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when fieldNameSizeOverride is within UPLOAD_BODY_LIMIT bytes", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldNameSizeOverride: 5 * 1024 * 1024 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("does not warn when fieldNameSizeOverride equals UPLOAD_BODY_LIMIT bytes", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldNameSizeOverride: 10485760 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("does not warn when no fieldNameSizeOverride is provided", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("logs a warning when fieldNameSizeOverride exceeds UPLOAD_BODY_LIMIT bytes and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldNameSizeOverride: 10485760 + 1 });

    expect(loggerMock.warn).toHaveBeenCalledOnce();
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        fieldNameSizeOverride: 10485760 + 1,
        UPLOAD_BODY_LIMIT: "10mb",
        UPLOAD_BODY_LIMIT_bytes: 10485760,
      }),
  ...
  it("does not throw when fieldNameSizeOverride exceeds UPLOAD_BODY_LIMIT bytes and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when fieldNameSizeOverride exceeds UPLOAD_BODY_LIMIT bytes and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("does not throw when fieldNameSizeOverride is within limit and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
...
// ---------------------------------------------------------------------------
// createUpload — fileSizeOverride vs UPLOAD_MAX_FILE_SIZE guard
// ---------------------------------------------------------------------------

describe("createUpload — fileSizeOverride vs UPLOAD_MAX_FILE_SIZE check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when fileSizeOverride equals UPLOAD_MAX_FILE_SIZE", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("logs a warning when fileSizeOverride is below UPLOAD_MAX_FILE_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fileSizeOverride: 1024 * 1024 });

    expect(loggerMock.warn).toHaveBeenCalledOnce();
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        fileSizeOverride: 1024 * 1024,
        UPLOAD_MAX_FILE_SIZE: 5 * 1024 * 1024,
      }),
  ...
  it("does not throw when fileSizeOverride is below UPLOAD_MAX_FILE_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when fileSizeOverride is below UPLOAD_MAX_FILE_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() =>
      cu(multer.memoryStorage(), { fileSizeOverride: 1024 * 1024 }),
    ).toThrow(/\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_FILE_SIZE/);
  });
});

// ---------------------------------------------------------------------------
// createUpload — fieldSizeOverride vs UPLOAD_FIELD_SIZE guard
// ---------------------------------------------------------------------------

describe("createUpload — fieldSizeOverride vs UPLOAD_FIELD_SIZE check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when fieldSizeOverride equals UPLOAD_FIELD_SIZE", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldSizeOverride: 1048576 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("logs a warning when fieldSizeOverride is below UPLOAD_FIELD_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldSizeOverride: 512 * 1024 });

    expect(loggerMock.warn).toHaveBeenCalledOnce();
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        fieldSizeOverride: 512 * 1024,
        UPLOAD_FIELD_SIZE: 1048576,
      }),
  ...
  it("does not throw when fieldSizeOverride is below UPLOAD_FIELD_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when fieldSizeOverride is below UPLOAD_FIELD_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() =>
      cu(multer.memoryStorage(), { fieldSizeOverride: 512 * 1024 }),
    ).toThrow(/\[UPLOAD_STRICT_CONFIG\].*UPLOAD_FIELD_SIZE/);
  });
  ...
  it("logs a warning when fieldSizeOverride exceeds UPLOAD_FIELD_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldSizeOverride: 1048576 + 1 });

    expect(loggerMock.warn).toHaveBeenCalledOnce();
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        fieldSizeOverride: 1048576 + 1,
        UPLOAD_FIELD_SIZE: 1048576,
      }),
      expect.stringContaining("UPLOAD_FIELD_SIZE"),
    );
  ...
  it("does not throw when fieldSizeOverride exceeds UPLOAD_FIELD_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when fieldSizeOverride exceeds UPLOAD_FIELD_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("does not throw when fieldSizeOverride equals UPLOAD_FIELD_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
...
// ---------------------------------------------------------------------------
// createUpload — fieldNameSizeOverride vs UPLOAD_FIELD_NAME_SIZE guard
// ---------------------------------------------------------------------------

describe("createUpload — fieldNameSizeOverride vs UPLOAD_FIELD_NAME_SIZE check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when fieldNameSizeOverride equals UPLOAD_FIELD_NAME_SIZE", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldNameSizeOverride: 100 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("does not warn when fieldNameSizeOverride is below UPLOAD_FIELD_NAME_SIZE", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("logs a warning when fieldNameSizeOverride exceeds UPLOAD_FIELD_NAME_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldNameSizeOverride: 101 });

    expect(loggerMock.warn).toHaveBeenCalledOnce();
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        fieldNameSizeOverride: 101,
        UPLOAD_FIELD_NAME_SIZE: 100,
      }),
  ...
  it("does not throw when fieldNameSizeOverride exceeds UPLOAD_FIELD_NAME_SIZE and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when fieldNameSizeOverride exceeds UPLOAD_FIELD_NAME_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() =>
      cu(multer.memoryStorage(), { fieldNameSizeOverride: 101 }),
    ).toThrow(/\[UPLOAD_STRICT_CONFIG\].*UPLOAD_FIELD_NAME_SIZE/);
  });

  it("does not throw when fieldNameSizeOverride equals UPLOAD_FIELD_NAME_SIZE and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
...
// ---------------------------------------------------------------------------
// createUpload — fieldNameSizeOverride vs fieldSizeOverride cross-property guard
// ---------------------------------------------------------------------------

describe("createUpload — fieldNameSizeOverride vs fieldSizeOverride cross-property check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when both are provided and fieldNameSizeOverride equals fieldSizeOverride", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), {
      fieldNameSizeOverride: 100,
      fieldSizeOverride: 100,
    });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("does not warn when both are provided and fieldNameSizeOverride is below fieldSizeOverride", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    vi.doMock("../config", () => ({
      ...
      UPLOAD_MAX_PARTS: 30,
      UPLOAD_FIELD_NAME_SIZE: 100,
      UPLOAD_FIELD_SIZE: 500,
    }));

    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), {
      fieldNameSizeOverride: 100,
      fieldSizeOverride: 500,
    });
  ...
  it("does not warn when only fieldSizeOverride is provided (no fieldNameSizeOverride to compare against)", async () => {
    ...
    vi.doMock("../config", () => ({
      ...
      UPLOAD_MAX_FIELDS: 20,
      UPLOAD_MAX_PARTS: 30,
      UPLOAD_FIELD_NAME_SIZE: 10,
      UPLOAD_FIELD_SIZE: 10,
    }));

    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { fieldSizeOverride: 10 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("logs a warning when both are provided and fieldNameSizeOverride exceeds fieldSizeOverride and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), {
      fieldNameSizeOverride: 500,
      fieldSizeOverride: 200,
    });
  ...
  it("does not throw when both are provided and fieldNameSizeOverride exceeds fieldSizeOverride and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("throws when both are provided and fieldNameSizeOverride exceeds fieldSizeOverride and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  it("does not throw when both are provided, fieldNameSizeOverride equals fieldSizeOverride, and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
...
// ---------------------------------------------------------------------------
// createUpload — maxFiles vs UPLOAD_MAX_FILES below-baseline guard
// ---------------------------------------------------------------------------

describe("createUpload — maxFiles below UPLOAD_MAX_FILES check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when maxFiles equals UPLOAD_MAX_FILES", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFiles: 10 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("does not warn when maxFiles exceeds UPLOAD_MAX_FILES", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFiles: 20 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("logs a warning when maxFiles is below UPLOAD_MAX_FILES and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFiles: 3 });
    ...
    expect(loggerMock.warn).toHaveBeenCalledWith(
      ...
      expect.stringContaining("UPLOAD_MAX_FILES"),
    );
  });

  it("does not throw when maxFiles is below UPLOAD_MAX_FILES and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFiles: 3 })).not.toThrow();
  });

  it("throws when maxFiles is below UPLOAD_MAX_FILES and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFiles: 3 })).toThrow(
      /\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_FILES/,
    );
  });
});

// ---------------------------------------------------------------------------
// createUpload — maxFields vs UPLOAD_MAX_FIELDS below-baseline guard
// ---------------------------------------------------------------------------

describe("createUpload — maxFields below UPLOAD_MAX_FIELDS check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when maxFields equals UPLOAD_MAX_FIELDS", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFields: 20 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("logs a warning when maxFields exceeds UPLOAD_MAX_FIELDS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFields: 50 });
    ...
    expect(loggerMock.warn).toHaveBeenCalledWith(
      ...
      expect.stringContaining("UPLOAD_MAX_FIELDS"),
    );
  });

  it("does not throw when maxFields exceeds UPLOAD_MAX_FIELDS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFields: 50 })).not.toThrow();
  });

  it("throws when maxFields exceeds UPLOAD_MAX_FIELDS and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFields: 50 })).toThrow(
      /\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_FIELDS/,
    );
  });

  it("logs a warning when maxFields is below UPLOAD_MAX_FIELDS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxFields: 5 });
  ...
  it("does not throw when maxFields is below UPLOAD_MAX_FIELDS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFields: 5 })).not.toThrow();
  });

  it("throws when maxFields is below UPLOAD_MAX_FIELDS and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxFields: 5 })).toThrow(
      /\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_FIELDS/,
    );
  });
});
...
// ---------------------------------------------------------------------------
// createUpload — maxParts vs UPLOAD_MAX_PARTS below-baseline guard
// ---------------------------------------------------------------------------

describe("createUpload — maxParts below UPLOAD_MAX_PARTS check", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it("does not warn when maxParts equals UPLOAD_MAX_PARTS", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxParts: 30 });

    expect(loggerMock.warn).not.toHaveBeenCalled();
  });

  it("logs a warning when maxParts exceeds UPLOAD_MAX_PARTS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxParts: 60 });
  ...
  it("does not throw when maxParts exceeds UPLOAD_MAX_PARTS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxParts: 60 })).not.toThrow();
  });

  it("throws when maxParts exceeds UPLOAD_MAX_PARTS and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxParts: 60 })).toThrow(
      /\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_PARTS/,
    );
  });

  it("logs a warning when maxParts is below UPLOAD_MAX_PARTS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    const { createUpload: cu } = await import("./upload");
    cu(multer.memoryStorage(), { maxParts: 8 });
  ...
  it("does not throw when maxParts is below UPLOAD_MAX_PARTS and strict mode is off", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxParts: 8 })).not.toThrow();
  });

  it("throws when maxParts is below UPLOAD_MAX_PARTS and UPLOAD_STRICT_CONFIG is true", async () => {
    const loggerMock = {
      warn: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
    ...
    expect(() => cu(multer.memoryStorage(), { maxParts: 8 })).toThrow(
      /\[UPLOAD_STRICT_CONFIG\].*UPLOAD_MAX_PARTS/,
    );
  });
});
