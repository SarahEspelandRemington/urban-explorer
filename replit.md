...
- **Crash reporting** (`artifacts/urban-explorer/lib/sentry.ts`, `lib/sentryWalk.ts`):
  ...
  - Sentry metrics: `trackNarrationFallback(reason)` increments `narration.audio_fallback` counter with a `reason` tag to track TTS fallback rates. The `reason` tag covers BOTH fetch-side failures (`write_failure | endpoint_error | bad_response` — emitted from `lib/fetchNarrationPayload.ts` when the audio bytes never reach the playback engine) AND playback-side silent-skip failures (`playback_create | playback_play | playback_status_error | playback_watchdog` — emitted from `hooks/useNarration.ts` when expo-audio receives the audio but cannot play it). `trackPrefetchEvent(event)` increments `narration.prefetch_event` counter (tagged `event` ∈ `HIT | MISS | STALE_DISCARD | STOP_WALK_DISCARD | DEDUPE`) for every narration prefetch outcome — see "Sentry Dashboards & Alerts" below for the saved panel and regression alert wired off this metric.
