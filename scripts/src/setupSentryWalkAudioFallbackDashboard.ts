...
async function createAlertRule(
  ...
  const name = "Walk Mode audio fallback rate";
  ...
  // equation across two metrics. We alert on absolute fallback volume here
  // (sum(narration.audio_fallback) over 1h). The thresholds (15 critical,
  // 8 warning) approximate a > 10% rate against the typical hourly walk
  // volume seen in production; tune in the Sentry UI once real traffic
  // settles. The dashboard's "Audio fallback rate %" panel carries the
...
async function patchReplitMd(
  ...
  const alertLine = `    - **Audio Fallback Alert URL**: ${alertUrl}`;
  ...
  const after = before
    ...
    .replace(/^ {4}- \*\*Audio Fallback Alert URL\*\*:.*$/m, alertLine);

  if (after === before) {
    console.warn(
      "replit.md was not modified — could not find the Audio Fallback Dashboard URL / Alert URL placeholder lines. " +
  ...
  console.log(`Updated replit.md with audio-fallback dashboard + alert URLs.`);
}
